(function () {
  ...
  var insta_slider_three = new Swiper(".insta-slider-three", {
    loop: false,
    speed: 15000,
    freemode: false,
    spaceBetween: 20,
...
function startCountdown(element, endDate) {
  function update() {
    ...
    if (timeLeft <= 0) {
      element.innerHTML =
        "<div class='cdown'><span class='time-count'>0</span> <p>Time's Up!</p></div>";
      clearInterval(timer);
      return;
    }
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function initializePriceRangeSlider() {
    ...
    minPriceSlider.addEventListener("input", function () {
      if (
        parseInt(minPriceSlider.value, 10) > parseInt(maxPriceSlider.value, 10)
      ) {
    ...
    maxPriceSlider.addEventListener("input", function () {
      if (
        parseInt(maxPriceSlider.value, 10) < parseInt(minPriceSlider.value, 10)
      ) {
  ...
  if (
    ...
    console.log(
      "Price range slider not initialized: required elements not found."
    );
  }
});
...
try {
  ...
  function setTheme(themeName) {
    localStorage.setItem("pinkbakery_theme", themeName);
    document.documentElement.className = themeName;
  }
  // function to toggle between light and dark theme
  function toggleTheme() {
    if (localStorage.getItem("pinkbakery_theme") === "theme-dark") {
